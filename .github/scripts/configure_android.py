...
// Several plugins (geolocator_android among them) ship without declaring a
// compileSdk and inherit nothing, which fails configuration with
// "compileSdkVersion is not specified". Forcing one SDK level across every
// subproject is the standard remedy.
//
// This block must come before Flutter's `subprojects { evaluationDependsOn }`,
// because afterEvaluate cannot be registered on an already-evaluated project.
// The state check below keeps it working even if that ordering changes.
subprojects {
    def applyMistrioSdk = {
    ...
    if (project.state.executed) {
        ...
        project.afterEvaluate { applyMistrioSdk() }
    }
}
"""
...
def patch_root() -> None:
    ...
    # Order matters. Flutter generates `subprojects { evaluationDependsOn(":app") }`
    # which evaluates projects eagerly, and afterEvaluate cannot be registered on a
    # project that has already been evaluated. Putting this block first guarantees
    # registration happens before anything triggers evaluation.
    path.write_text(block.lstrip("\n") + "\n" + text)
    print(f"  patched {path} (prepended)")
...
def patch_main_activity() -> None:
    """
    `flutter create --org com.mistrio --project-name mistrio` puts MainActivity
    in package com.mistrio.mistrio. Once the namespace is rewritten to
    com.mistrio.user, the manifest's `.MainActivity` resolves to a class that
    does not exist and the app dies with ClassNotFoundException the moment it
    is opened. Move the source file to match.
    """
    src_root = ANDROID / "app/src/main"

    found = list(src_root.rglob("MainActivity.kt")) + list(src_root.rglob("MainActivity.java"))
    if not found:
        print("  ! MainActivity not found, skipping")
        return

    current = found[0]
    is_kotlin = current.suffix == ".kt"
    lang_dir = src_root / ("kotlin" if is_kotlin else "java")
    target = lang_dir.joinpath(*APP_ID.split(".")) / current.name

    text = current.read_text()
    text = re.sub(r'^package\s+[\w.]+', f"package {APP_ID}", text, count=1, flags=re.M)

    if target.resolve() == current.resolve():
        current.write_text(text)
        print(f"  MainActivity already at {target}")
        return
    ...
    # clean up the now-empty old package directories
    parent = current.parent
    while parent != lang_dir and parent.exists() and not any(parent.iterdir()):
        parent.rmdir()
        parent = parent.parent

    print(f"  moved MainActivity to {target}")
...
def patch_manifest() -> None:
    ...
    # Fully qualify the activity so it cannot be misresolved against the
    # rewritten namespace.
    text = text.replace('android:name=".MainActivity"',
                        f'android:name="{APP_ID}.MainActivity"')
